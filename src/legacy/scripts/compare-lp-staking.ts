/**
 * Compare old vs new LP Staking reward calculators.
 *
 * Usage:
 *   npx ts-node src/scripts/compare-lp-staking.ts
 */
...
async function run() {
  const { calculateLpStakingRewards } = require("../modules/lp-staking-rewards");
  const { calculateLpStakingRewardsV2 } = require("../modules/lp-staking-rewards-v2");
  const { config } = require("../config");

  const cfg = config();

  console.log("=== LP Staking Calculator Comparison ===\n");
  console.log(`Windows: ${cfg.rewards.lpStaking.windows.length}`);
  console.log(`LP_STAKING_START_BLOCK: ${cfg.LP_STAKING_START_BLOCK}`);
  console.log(`LP_STAKING_END_BLOCK: ${cfg.LP_STAKING_END_BLOCK}\n`);

  // Run old calculator
  console.log("Running OLD calculator...");
  const oldStart = Date.now();
  const oldResult = await calculateLpStakingRewards(
    cfg.LP_STAKING_START_BLOCK,
    cfg.LP_STAKING_END_BLOCK
  );
  ...
  const newResult = await calculateLpStakingRewardsV2(
    cfg.LP_STAKING_START_BLOCK,
    cfg.LP_STAKING_END_BLOCK
  );
  ...
  // Compare across all rewardToken / stakingType combos
  ...
  for (const token of allTokens) {
    ...
    for (const stakingType of allTypes) {
      const oldUsers = oldTokenResult[stakingType] || {};
      const newUsers = newTokenResult[stakingType] || {};
      ...
      console.log(`  Diff: ${Math.abs(oldTotal - newTotal)}`);
