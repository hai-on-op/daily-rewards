/**
 * Compare old vs new haiVELO reward calculators.
 *
 * Usage:
 *   npx ts-node src/scripts/compare-haivelo.ts
 */
...
async function run() {
  const { calculateHaiveloRewards } = require("../modules/haivelo-rewards");
  const {
    calculateHaiveloRewardsV2,
  } = require("../modules/haivelo-rewards-v2");

  const rewardAmount = 1000;

  console.log("=== haiVELO Calculator Comparison ===\n");
  console.log(`Reward amount: ${rewardAmount}\n`);

  // Run old calculator
  console.log("Running OLD calculator...");
  const oldStart = Date.now();
  const oldUsers = await calculateHaiveloRewards(rewardAmount);
  ...
  const newUsers = await calculateHaiveloRewardsV2(rewardAmount);
